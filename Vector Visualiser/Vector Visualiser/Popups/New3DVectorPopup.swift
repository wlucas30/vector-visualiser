//
//  New3DVectorPopup.swift
//  Vector Visualiser
//
//  Created by Will Lucas on 05/08/2024.
//

import SwiftUI

struct New3DVectorPopup: View {
    @State private var scale: CGFloat = 0
    @State private var iScalar: String = ""
    @State private var jScalar: String = ""
    @State private var kScalar: String = ""
                            ...
                            VStack {
                                ...
                                VStack(alignment: .leading) {
                                    ...
                                    TextField("Basis vector j", text: $jScalar)
                                        .frame(width: metrics.size.width * 0.2)
                                    TextField("Basis vector k", text: $kScalar)
                                        .frame(width: metrics.size.width * 0.2)
    ...
    private func addNewVector() {
        errorOccurred = nil
        if let i = Int(iScalar), let j = Int(jScalar), let k = Int(kScalar) {
            vectors.append(Vector3D(id: UUID(), i: CGFloat(i), j: CGFloat(j), k: CGFloat(k), isUnitVector: false))
            currentPopup = .none
        } else {
